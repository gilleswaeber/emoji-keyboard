...
export async function nodeBuild({ucdVersion, emojiVersion, cldrVersion}: {ucdVersion: string, emojiVersion: string, cldrVersion: string}): Promise<ConsolidatedUnicodeData> {
	const resources: UnicodeResources = {
		emojiTest: memoizeOne(() => fetch(`https://unicode.org/Public/emoji/${emojiVersion}/emoji-test.txt`).then(r => r.text())),
